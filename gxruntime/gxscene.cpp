...
n_texs(0),tris_drawn(0){
	...
	can_wb=false;
	hw_tex_stages=1;
	D3DCAPS9 devDesc;//D3DDEVICEDESC7
	if( dir3dDev->GetDeviceCaps( &devDesc )>=0 ){
		DWORD caps=devDesc.RasterCaps;//dpcTriCaps.dw
		//texture stages
	...
	caps_level=100;
	if( devDesc.TextureCaps & D3DPTEXTURECAPS_CUBEMAP ){//.dpcTriCaps
		caps_level=110;
	}
...
IDirect3DBaseTexture9* main_surf;
ddSurf* surf;

IDirect3DBaseTexture9* getTexSur()
{
...
void gxScene::setTexState( int n,const TexState &state,bool tex_blend ){
	...
	//set canvas
	dir3dDev->SetTexture( n,getTexSur() );//state.canvas->
...
void gxScene::setAntialias( bool n ){
	...
	//antialias=n;setRS(D3DANTIALIAS_ANTIALIAS,antialias ? D3DANTIALIAS_SORTINDEPENDENT : D3DANTIALIAS_NONE );
}

void gxScene::setWireframe( bool n ){
	if( n==wireframe ) return;
	wireframe=n;setRS(D3DRS_FILLMODE,wireframe ? D3DFILL_WIREFRAME : D3DFILL_SOLID );
}
